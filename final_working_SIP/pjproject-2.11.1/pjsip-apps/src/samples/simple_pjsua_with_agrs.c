...
int main(int argc, char *argv[])
{
    ...
    int opt;
    unsigned char username[50]={0};
    unsigned char password[100]={0};
    unsigned char realm[20]={0};
    unsigned char domain[50]={0};
    ...
    { 
        ...
        { 
            ...
            case 'p': 
                ...
                break; 
            case 'd': 
                printf("domain : %s",optarg); 
                strcpy(domain,optarg);
                break; 
        }
    }
    
    unsigned char identity[100]={0};
    unsigned char url[100]={0};
    
    sprintf((char*)identity,"sip:%s@%s",username,domain);
    printf("\nidentity : %s\n",identity); fflush(stdout);
    
    sprintf((char*)url,"sip:%s",domain);
    printf("\nurl : %s\n",url); fflush(stdout);

    //~ exit(1);

	pjsua_acc_config_default(&cfg);
	cfg.id = pj_str(identity);
